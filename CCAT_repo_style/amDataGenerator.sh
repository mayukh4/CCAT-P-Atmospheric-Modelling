...
am CCAT_annual_50.amc  0 GHz  1000 GHz  10 MHz  45 deg  1.0 >/dev/null 2> ACT_annual_50.45.err
...
CerroConfigQ2=$(bc <<<"scale=10; $ACTConfigQ2")
...
if [ "$fullData" = true ] ; then
    ...
    do
        for s in {1..1}
        do
            am CCAT_annual_50.amc  0 GHz  1000 GHz  10 MHz  $((i)) deg  $(bc <<<"scale=10; $CerroConfigQ2/20*$((s))") > CCAT_annual_$((s)).$((i)).out 2>/dev/null
        done
